...
- **Read the card before the code.** When any decision in this repository depends on what a card does, read the card's text first and reconcile the code to it; the card's reading determines the way forward. Derive the expected outcome from the Oracle text clause by clause, then read the code to find where it diverges. A possibility the code admits is not a behavior until a card's text produces it, and when card and code disagree, the card is right and the code is the defect. This is the companion of "Verify the card, not just the rule": that bullet confirms the premise is real; this one keeps every decision resting on it anchored to the card.
...
2. **Teammates cannot spawn subagents.** All review subagents must be spawned by the lead. The lead receives the plan/implementation from the teammate, spawns a review subagent (model: opus), and sends feedback back to the teammate. For this team feedback loop, choose a cap before starting: one round for a routine, localized change, up to three for cross-cutting work. If the cap is reached with findings, stop and hand the remaining findings to the lead; do not treat the cap as acceptance or open a PR with an unresolved review. A skill with its own review-loop and escalation rules (such as [`engine-implementer`](.claude/skills/engine-implementer/SKILL.md)) governs its own pipeline. Preserve progress using that workflow's checkpoint commits before verification and review; a checkpoint is not a clean-review result. The final committed-head review required by [CONTRIBUTING.md](CONTRIBUTING.md) still applies.
