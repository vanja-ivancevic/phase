...
fn quantity_ref_contains_filter_prop(
    ...
    match qty {
        ...
        | QuantityRef::PreviousDamageAmountCappedByTargetPreDamageValue => false,
    }
}
...
fn zone_change_filter_inner(
    ...
    let scoped_iteration_player = ctx.scoped_iteration_player;
    ...
    match filter {
        ...
        TargetFilter::Not { filter: inner } => {
            !zone_change_filter_inner(state, record, inner, ctx)
        }
...
fn chosen_color_matches(
    ...
    if source.ability.is_some() {
        if let Some(ChoiceValue::Color(color)) = state.last_named_choice.as_ref() {
            return candidate_colors.contains(color);
        }
    }
...
mod tests {
    ...
    /// A permanent that chooses a color again uses its current answer for
    /// "of the chosen color", not the first answer still stored for linked
    /// abilities (CR 607.2d).
    #[test]
    fn chosen_color_filter_uses_latest_source_choice() {
        let mut state = setup();
        let source = add_creature(&mut state, PlayerId(0), "Source");
        let red = add_creature(&mut state, PlayerId(0), "Red");
        let blue = add_creature(&mut state, PlayerId(0), "Blue");
        state.objects.get_mut(&red).unwrap().color = vec![ManaColor::Red];
        state.objects.get_mut(&blue).unwrap().color = vec![ManaColor::Blue];
        let filter =
            TargetFilter::Typed(TypedFilter::default().properties(vec![FilterProp::IsChosenColor]));
