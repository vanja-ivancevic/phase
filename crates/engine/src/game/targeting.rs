...
/// matching, `effects::resolved_effect_object_ids`, and `sacrifice`'s slot arm.
/// Callers that still read `resolved_targets`' whole-chain return for a
/// `ParentTargetSlot` filter (first object or whole list) bypass it: destroy,
/// bounce, counter, put-on-top-or-bottom, exchange control, pair with, change
/// targets, the damage-replacement filters, gain control's give, and the
