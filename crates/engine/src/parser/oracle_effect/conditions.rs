...
/// CR 608.2b: the printed "if both/all targets are still legal as this
/// spell/ability resolves" legality rider, and the one authority for its phrase
/// vocabulary. `text` must already be lowercase.
///
/// Returns whether a rider phrase was present, together with the text with every
/// occurrence removed. Two consumers, so the vocabulary is spelled once:
/// * `parser::swallow_check` discharges the rider from its `Condition_If` audit
///   using the residual;
/// * `oracle_effect::mod::apply_target_legality_rider_from_text` attaches the
///   runtime gate the phrase abbreviates.
pub(crate) fn strip_target_legality_rider_phrases(text: &str) -> (bool, String) {
    let mut residual = text.to_owned();
...
pub(crate) fn ability_condition_to_static_condition(
    ...
    match ac {
        ...
        | AbilityCondition::AllDeclaredTargetsLegal
