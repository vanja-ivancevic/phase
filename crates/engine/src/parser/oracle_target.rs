...
pub fn parse_target_with_syntax<'a>(
    ...
    if let Ok((after_target, _)) = tag::<_, _, OracleError<'_>>("target ").parse(lower.as_str()) {
        ...
        // A single controlled-count player clause can be parsed here. A
        // coordinated relative clause belongs to the shared conjunct grammar
        // below; consuming only its first predicate would make the target
        // under-restricted at announcement (CR 601.2c).
        ...
        if let Some((head, relation, _)) = player_head {
            ...
            if let Some((comparator, count, bare_filter, remainder)) =
                parse_controls_permanent_object(original_clause, &mut tentative_ctx)
            {
        ...
        .parse(after_target)
        {
                ...
                )
            {
                ...
                // A relative clause was recognized, but its remainder did not
                // end at a real clause boundary. Decline the whole target
                // rather than returning the bare player and handing the
                // unconsumed restriction back to the effect parser; that
                // under-restricted fallback can be retried indefinitely.
                return (TargetFilter::Any, text, syntax);
            }

            // The input visibly starts a relative player clause, but none of
            // the modeled productions consumed it in full. Fail closed for
            // the same reason as the partial-success case above.
            if after_noun.starts_with("who ") || after_noun.starts_with("with ") {
                return (TargetFilter::Any, text, syntax);
            }
